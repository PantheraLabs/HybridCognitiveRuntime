...
const getRedirectUri = () => {
  const envUri = import.meta.env.VITE_GITHUB_REDIRECT_URI;
  return envUri || `${window.location.origin}/auth/github/callback`;
};

const buildStateToken = () => {
...
export default function Auth() {
  ...
  const isLogin = useMemo(() => window.location.pathname === '/login', []);

  const handleGitHubAuth = () => {
    const clientId = import.meta.env.VITE_GITHUB_CLIENT_ID;
    if (!clientId) {
      setError('GitHub OAuth client ID is missing. Update your .env configuration.');
      return;
    }
    ...
    sessionStorage.setItem('github_oauth_next', window.location.pathname === '/signup' ? '/app' : window.location.pathname);
        ...
        <div className="bg-[#111111] border border-white/5 rounded-3xl p-8 shadow-2xl">
           <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
              ...
              <button
                ...
                className="w-full h-12 bg-white hover:bg-zinc-100 text-black font-semibold rounded-xl flex items-center justify-center gap-3 transition-all active:scale-[0.98]"
              >
              ...
              {error && (
                <div className="text-red-400 text-sm text-center bg-red-500/10 border border-red-500/30 rounded-xl py-2">
                  {error}
                </div>
              )}
